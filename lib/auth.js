...
function getSecret() {
  // Uretimde SESSION_SECRET mutlaka ortam degiskeni olarak set edilmeli (AWS Amplify'da
  // Environment variables bolumunden). Yerelde unutulursa site yine calissin diye bir varsayilan var.
...
// ---- admin oturumu: normal kullanici hesaplarindan tamamen ayri, ----
// ---- ADMIN_EMAIL/ADMIN_PASSWORD ortam degiskenleriyle dogrulanir  ----
const ADMIN_COOKIE_NAME = "havasite_admin_session";
const ADMIN_SESSION_TTL_MS = 12 * 3600 * 1000; // 12 saat (musteri oturumundan daha kisa)

function createAdminSessionCookie(req) {
  const token = sign({ admin: true, exp: Date.now() + ADMIN_SESSION_TTL_MS });
  const secure = isSecureRequest(req) ? "; Secure" : "";
  return `${ADMIN_COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; Path=/; Max-Age=${Math.floor(ADMIN_SESSION_TTL_MS / 1000)}; SameSite=Lax${secure}`;
}
...
function readAdminSession(req) {
  const cookies = parseCookies(req.headers.cookie);
  const session = verify(cookies[ADMIN_COOKIE_NAME]);
  return !!(session && session.admin);
}

// sabit-zamanli karsilastirma: uzunluklari farkli olsa bile hicbir zaman firlatmaz
// (girilen sifrenin uzunlugunu zamanlama farkindan sizdirmamak icin)
function timingSafeEqualStr(a, b) {
  const bufA = Buffer.from(String(a || ""));
  const bufB = Buffer.from(String(b || ""));
  if (bufA.length !== bufB.length) {
    // farkli uzunlukta da sabit sureli bir karsilastirma yap (erken donme yok)
